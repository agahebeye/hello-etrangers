...
watch(filters, (value) => {
    Inertia.get(route('foreigners.index'), pickBy(filters.value), { preserveState: true })
}, { deep: true })
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 v-if="filters.role" class="text-xl font-semibold leading-tight text-gray-800">
                {{ filters.role }}s
            </h2>
                    ...
                    <Dropdown align="right" width="48">
                        ...
                        <template #content>
                            <div class="flex flex-col">
                                <DropdownLink v-for="citizenship in [...new Set(citizenships)]" class="no-underline" @click="filters.citizenship = citizenship">
                                    {{ citizenship }}
                                </DropdownLink>
                            </div>
                        </template>
                    ...
                    <table class="relative w-full m-0 whitespace-no-wrap bg-white border-collapse table-auto table-striped">
                        <thead>
                            <tr class="text-left">
                                ...
                                <th class="sticky top-0 px-6 py-2 text-xs font-bold tracking-wider text-gray-600 uppercase bg-gray-100 border-b border-gray-200" v-if="!filters.role">Role</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template v-for="foreigner in foreigners.data">
                                <tr>
                                    ...
                                    <td v-if="!filters.role" class="border-t border-gray-200 border-dashed phoneNumber">
